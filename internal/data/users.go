...
// version of the password for a user. The plaintext field is a *pointer* to a string,
// so that we're able too distinguish between a plaintext password not being present in 
...
var (
	...
	ErrEditConflict = errors.New("error conflict")
)
...
func (m UserModel) Insert(user *User) error {
	...
	// We check for this error specifically, and return custom ErrDuplicateEmail error instead.
...
func(m UserModel) Update(user *User) error {
	query := `
		UPDATE users
		SET name = $1, email = $2, password_hash = $3, activvated =$4, version = version + 1
